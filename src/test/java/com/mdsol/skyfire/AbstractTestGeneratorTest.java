...
public class AbstractTestGeneratorTest {
	...
	/*
	 * String[] edges = stateMachine.getEdges().split("\n"); for(String edge : edges){ String[] vertices = edge.split(" ");
	 * //System.out.println(vertices[0]+ " :: " + vertices[1]); System.out.println(stateMachine.getReversedStateMappings().get(vertices[0]).getName() +
    ...
    @Test
    public void testGetTestPathsForRoc() throws IOException, InvalidInputException, InvalidGraphException {
	...
	paths = AbstractTestGenerator.getTestPaths(stateMachine.getEdges(), stateMachine.getInitialStates(), stateMachine.getFinalStates(), TestCoverageCriteria.EDGECOVERAGE);
