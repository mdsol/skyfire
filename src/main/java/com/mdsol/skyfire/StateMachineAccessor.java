...
public class StateMachineAccessor extends ModelAccessor {
    ...
    /**
     * Creates the one mapping from vertexes to integers and another mapping from integers to vertexes. This method cannot be overridden.
     * 
     * @param region
     *            a {@link org.eclipse.uml2.uml.Region} Region object
     */
	...
	for (Transition transition : firstLevelTransitions) {
		...
		if (transition.getSource() instanceof Pseudostate || transition.getSource() instanceof FinalState || transition.getSource() instanceof State
		    ...
		    if (!((transition.getSource() instanceof State && ((State) transition.getSource()).isComposite())
			    || (transition.getTarget() instanceof State && ((State) transition.getTarget()).isComposite()))) {
			edges = updateEdges(edges, transition);
			// firstLevelTransitions.add(transition);
			getTransitions().add(transition);
		    } else if ((transition.getSource() instanceof State && ((State) transition.getSource()).isComposite())
			    || (transition.getTarget() instanceof State && ((State) transition.getTarget()).isComposite())) {
	...
	if (transCompoState.size() > 0) {
		...
		if (t.getSource() instanceof State && ((State) t.getSource()).isComposite()) {
			...
			/*
			 * I decide not to create a transition from an initial state of a composite state to other states since the initial state is not a
			 * stable state for(Pseudostate initialState : sourceInitialStates){
			 * 
			 * Transition tempTran = region.createTransition(t.getName()); tempTran.setSource(initialState); tempTran.setTarget(t.getTarget());
			 * 
			 * transCompoState.add(tempTran); }
			 */
		    }
			...
			/*
			 * final states of a composite state is not connected to the coming transitions. for(FinalState finalState : targetFinalStates){
			 * 
			 * Transition tempTran = region.createTransition(t.getName()); tempTran.setSource(t.getSource()); tempTran.setTarget(finalState);
			 * 
			 * transCompoState.add(tempTran); }
			 */
		    }
		...
		} else if ((t.getSource() instanceof Pseudostate && ((Pseudostate) t.getSource()).getKind() == PseudostateKind.INITIAL_LITERAL)
			&& (t.getTarget() instanceof State && ((State) t.getTarget()).isSimple())) {
		    simpleTrans.add(t);
		    getTransitions().add(t);
		    transCompoState.remove(0);
		    edges = updateEdges(edges, t);
		} else if ((t.getTarget() instanceof Pseudostate && ((Pseudostate) t.getTarget()).getKind() == PseudostateKind.INITIAL_LITERAL)
			...
			&& (t.getTarget() instanceof Pseudostate && ((Pseudostate) t.getTarget()).getKind() == PseudostateKind.INITIAL_LITERAL)) {
		    simpleTrans.add(t);
		    getTransitions().add(t);
		    transCompoState.remove(0);
		    edges = updateEdges(edges, t);
		...
		/*
		 * System.out.println(simpleTrans.size()); for(Transition t1 : simpleTrans){ System.out.println(t1.getName() + " " + t1.getSource().getName() +
		 * " " + t1.getTarget().getName()); } System.out.println(transCompoState.size()); for(Transition t2 : transCompoState){
		 * //System.out.println(t2.getName() ); System.out.println(t2.getName() + " " + t2.getSource().getName() + " " + t2.getTarget().getName() +
		 * (t2.getSource() instanceof State && ((State)t2.getSource()).isSimple()) + (t2.getTarget() instanceof State &&
		 * ((State)t2.getTarget()).isSimple())); }
		 */
	    } while (transCompoState.size() > 0);
	}
	/*
	 * System.out.println(getTransitions().size()); for(Transition t3 : getTransitions()){ System.out.println(t3.getSource().getName() + " " +
	 * t3.getTarget().getName()); }
	 */
    ...
    /**
     * Gets all objects of {@link org.eclipse.uml2.uml.State} except {@link org.eclipse.uml2.uml.FinalState} and {@link org.eclipse.uml2.uml.Pseudostate} in the
     * region
     * 
     * @param region
     ...
     * 
     *         Initial states in state machine diagrams are of Pseudostate, in this method, each object of Pseudostate is examined. If an object of Pseudostate
     *         is connected with other states, it is treated to be an initial state.
     */
    ...
    /**
     * Gets all identifiable elements that can be mapped from a region of a state machine. So far only transitions, constraints, and states are to be returned.
     * 
     * @param region
	...
	/*
	 * EList<Vertex> vertices = region.getSubvertices(); for(Vertex vertex: vertices){ boolean hasSameName = false; for(NamedElement element: result){
	 * if(element.getName().equals(vertex.getName())){ hasSameName = true; } }
	 * 
	 * if(hasSameName == false){ if(vertex instanceof Pseudostate){ EList<Transition> outgoings = vertex.getOutgoings(); if(outgoings.size() > 0){
	 * result.add(((Pseudostate)vertex)); } } else if(vertex instanceof FinalState){ result.add(((FinalState)vertex)); }else{ result.add(vertex); } }
	 * 
	 * }
	 */
	return result;
