...
public class StateMachineAccessor extends ModelAccessor {
	...
	private void createStateMappings(Region region){
		...
		//if all states are simple, execute the if block; otherwise, do the else block
		if(isSimple){
			for(Vertex vertex: vertices){
...
//					System.out.println(aVertex.toString());
					if(aVertex instanceof Pseudostate && ((Pseudostate)aVertex).getKind() == PseudostateKind.INITIAL_LITERAL){
						...
						// this composite state does not have any sub-vertices when the size is 0
...
//							System.out.println("t.getname: " + t.getName() + " : guard : " + t.getGuard());
							
//							tempTran.setGuard(t.getGuard());
//							
//							System.out.println("temp tran: " + tempTran.getGuard());
//
//							System.out.println(t.getGuard().getSpecification().stringValue());
//							
//							Constraint tempTranGuard = tempTran.createGuard("Test2ToTest1");
//							ValueSpecification vs = tempTranGuard.createSpecification("testSpec", t.getGuard().getSpecification().getType(), t.getGuard().getSpecification().eClass());
